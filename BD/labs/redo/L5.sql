...
DROP TABLE angajati_est CASCADE CONSTRAINTS;
DROP TABLE departamente_est CASCADE CONSTRAINTS;
...
CREATE TABLE angajati_est
      ( cod_ang number(4) CONSTRAINT pk_ang_est PRIMARY KEY,
        nume varchar2(20) CONSTRAINT nume_ang_est NOT NULL,
        prenume varchar2(20),
        email char(15) UNIQUE,
        data_ang date DEFAULT SYSDATE,
        job varchar2(10),
        cod_sef number(4),
        salariu number(8,2) CONSTRAINT salariu_ang_est NOT NULL,
        cod_dep number(2)
       );
...
R: pentru ca data_ang are valoare default sysdate, nu null
...
R: la salary precizia era mai mare si la comision erau valorile null
...
ALTER TABLE angajati_est
ADD CONSTRAINT fkey_ang_dep FOREIGN KEY(cod_dep) REFERENCES departamente_est(cod_dep);
...
CREATE TABLE angajati_est
    (cod_ang number(4) constraint pkey_ang primary key,
     nume varchar2(20) constraint nume_ang not null,
     prenume varchar2(20),
     email char(15) constraint email_unique_ang UNIQUE,
     data_ang date default sysdate,
     job varchar2(10),
     cod_sef number(4) constraint sef_ang references angajati_est(cod_ang), 
     salariu number(8, 2) constraint salariu_ang not null,
     cod_dep number(2) constraint fk_ang_dep references departamente_est(cod_dep), 
                       constraint cod_dep_poz_ang CHECK(cod_dep > 0),
     comision number(2,2),
     constraint nume_pren_unique UNIQUE(nume, prenume),
     constraint sal_check CHECK(salariu > 100 * comision)
     );
...
R: nu pt ca nu merge foreign keyul
...
ALTER TABLE departamente_est
ADD CONSTRAINT fk_dep_ang_est FOREIGN KEY (cod_director) references angajati_est(cod_ang) ON delete set null; 
