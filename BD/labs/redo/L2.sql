...
SELECT TO_DATE('07-03-2023') + 3
FROM dual; 

-- DE CE NU ESTE CORECTA VARIANTA ANTERIOARA? 

acum merge ca am pus to_date;
...
SELECT TRUNC(SYSDATE - TO_DATE ('10-07-2000', 'DD-MM-YYYY'))
FROM dual; 
...
from employees
where last_name like ('J%') OR last_name like ('M%') OR lower(last_name) like ('__a%')
...
SELECT TO_CHAR(SYSDATE + 5/(24*60), 'DD/MM HH24:MI:SS') "Data"
FROM DUAL;
